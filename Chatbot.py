...
DATA_TXT = "data.txt"
MODEL_FILE = "LLAMA_chatbot_Tensor_10M"
...
def generate_text(model, context=None, max_tokens=100, device=device):
    ...
    generation = model.generate(context, max_tokens)[0].tolist()
    
    return decode(generation)[len(context[0]):]

def parse_response(response):
    if "o:" in response:
        return response.split("o:")[0]
    elif "i:" in response:
        return response.split("i:")[0]
    else:
        return response
...
while True:
    print()
    question = input("User: ")
    response = generate_text(m, "i:" + question + "\no:", max_tokens=100)
    parsed_response = parse_response(response)
    print("Chatbot:", parsed_response + "\n")