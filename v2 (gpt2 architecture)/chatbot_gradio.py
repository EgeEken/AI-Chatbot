...
def chatbot(model, query, stream=True, returns=False, max_length=250):
    ...
    while x_next != enc.encode("\n")[0] and x.size(1) < max_length:
        ...
        if stream:
            token_str = enc.decode(x_next[0].tolist())
            print(token_str, end="")
            response_tokens.append(token_str)
    
    # Extract just the response part (after "o:")
    full_text = enc.decode(x[0].tolist())
    response_start = full_text.find("o:") + 2
    response = full_text[response_start:].strip()
    
    if returns or not stream:
        return response

def gradio_chatbot(message, history):
    """Function to handle Gradio chat interface"""
    try:
        response = chatbot(model, message, stream=True, returns=True, max_length=250)
        # Clean up the response by removing any unwanted tokens
...
model_filename = "gpt2_pretrained_llama3_io_model.pth"
model = GPT(GPTConfig())
...
if __name__ == "__main__":
    interface = gr.ChatInterface(
        fn=gradio_chatbot,
        title="AI Chatbot",
        description="Built by Ege Eken (github.com/EgeEken/AI-Chatbot). Ask a question!",
        examples=[
            "What is an arachnid?",
            "What are fruits?",
